...
def show_live_camera_feed():
    ...
    with st.sidebar:
        st.markdown("""
            ...
            """, unsafe_allow_html=True)
        st.video("C:/Users/yogan/Downloads/5439078-uhd_3840_2160_25fps.mp4", format="video/mp4")
...
class InterviewBot:
    ...
    def _control_video_playback(self, action: str):
        """Control video playback based on the action specified."""
        if action == 'play':
            st.session_state['video_play'] = True
            # Integrate wav2lipsync processing
            # Assuming we have a function process_with_wav2lipsync that handles the synchronization
            if 'video_frames' in st.session_state and 'audio_path' in st.session_state:
                video_frames = st.session_state['video_frames']
                audio_path = st.session_state['audio_path']
                # Process video frames with wav2lipsync
                synchronized_frames = process_with_wav2lipsync(video_frames, audio_path)
                st.session_state['synchronized_frames'] = synchronized_frames
        elif action == 'stop':
    ...
    def _text_to_speech(self, text: str):
        ...
        try:
            # Prepare video frames and audio for synchronization
            if 'video_frames' in self.session_state and 'audio_path' in self.session_state:
                video_frames = self.session_state['video_frames']
                audio_path = self.session_state['audio_path']
                # Process video frames with wav2lipsync
                synchronized_frames = process_with_wav2lipsync(video_frames, audio_path)
                self.session_state['synchronized_frames'] = synchronized_frames
                self._control_video_playback('play')  # Automatically start video playback
            ...
            self._control_video_playback('stop')  # Stop video after speaking
        except Exception as e:
...
def process_with_wav2lipsync(video_frames, audio_path):
    """Process video frames with Wav2Lip to synchronize lip movements with audio."""
    # Load the Wav2Lip model (assuming the model is already defined elsewhere)
    ...
    # Prepare video frames and mel spectrogram for processing
    processed_frames = []
    batch_size = 128  # Example batch size
    for i in range(0, len(video_frames), batch_size):
        img_batch = video_frames[i:i + batch_size]
        mel_batch = mel[:, i:i + batch_size]
        
        # Convert to tensors
        img_batch_tensor = torch.FloatTensor(np.transpose(img_batch, (0, 3, 1, 2))).to(device)
        mel_batch_tensor = torch.FloatTensor(np.transpose(mel_batch, (0, 3, 1, 2))).to(device)
        ...
        processed_frames.extend(pred.astype(np.uint8))

    return processed_frames

# Ensure the Wav2Lip model is loaded
